 ...
 */	
	...
	{
		$cf_dir = realpath('../../../../../');
		$indexPhp = file_get_contents($cf_dir.'/index.php');
		
		preg_match('@\$application_folder.*"(.+)"@i', $indexPhp, $match);
		$cfApp = (isset($match[1])) ? $match[1] : FALSE;
		if(!$cfApp) die('application files could not be found');
		
		set_include_path(get_include_path() . PATH_SEPARATOR . $cf_dir);
		$cf_config = $cf_dir.'/'.$cfApp . '/config/config.php';
		require_once ($cf_config);
		//$cf_config = file_get_contents($cf_config);
		
		//preg_match('@\$config\[\'(.+)\'\].*"(.+)"@i', $cf_config, $match);
		//$cfBaseUrl = (isset($match[1])) ? $match[1] : FALSE;
		//if(!$cfBaseUrl) die('Base URL could not be found');
		...
		{
			$this->$key = (isset($params[$key])) ? $params[$key] : $config[$key];
		}
...
$hash	 = substr($ses, strlen($ses)-32); // get last 32 chars
