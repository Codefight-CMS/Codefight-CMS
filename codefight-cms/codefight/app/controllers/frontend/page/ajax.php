		...
		if(isset($_POST))
		{
			if(isset($_POST['name']))
			{
				...
				else
				{
					...
					if($page_id) {
						...
						if($this->db->insert_id()) {
							...
							foreach(get_default_recipients() as $recipient)
							{
								$recipient = trim($recipient);
								if(!empty($recipient))
								{
									mail($recipient,'new comment-'.$_SERVER['HTTP_HOST'], "$name \n\r $comment \n\r $url");
								}
							}
						}
					}
				...
				if(is_array($q) && count($q) > 0) foreach($q as $v) {
					...
					$c .= "\n" . '<a rel="external nofollow" href="' . $v['url'] . '#" target="_blank">' . $v['name'] . '</a></p>';
					$c .= "\n" . '</div>';
				}
				echo $c;
			}
...
/* Location: ./app/frontend/controllers/page/ajax.php */
?>