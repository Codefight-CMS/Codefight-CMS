...
<div class="setting">
		<ul><?php
			foreach($setting as $v) { ?>
				...
				if($v['setting_key']=='default_template') { ?>
					...
					switch($v['setting_form']) {
						case 'radio':
							...
							break;
						case 'textbox': ?>
								<input name="<?php echo $v['setting_key'];?>" type="text" id="<?php echo $v['setting_key'];?>" value="<?php echo $v['setting_value']; ?>" size="50" /><?php
							break;
						case 'textarea': ?>
								<textarea name="<?php echo $v['setting_key'];?>" cols="35" rows="3" id="<?php echo $v['setting_key'];?>"><?php echo $v['setting_value']; ?></textarea>
<?php
							break;
						case 'select': 
							//split options separated with bar
